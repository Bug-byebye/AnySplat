...
def get_set_list(category_dir, split, is_single_sequence_subset=False):
    ...
    if is_single_sequence_subset:
        ...
        subset_list_files = [f for f in listfiles if "fewview_train" in f]
...
def prepare_sequences(
    ...
    for seq_name, frame_number, filepath in tqdm(sequences_all):
        ...
        f_data = frame_data_processed[seq_name][frame_number]
        focal_length = f_data["viewpoint"]["focal_length"]
        principal_point = f_data["viewpoint"]["principal_point"]
        image_size = f_data["image"]["size"]
        R, tvec, camera_intrinsics = opencv_from_cameras_projection(
            np.array(f_data["viewpoint"]["R"]),
            np.array(f_data["viewpoint"]["T"]),
            np.array(focal_length),
            np.array(principal_point),
            np.array(image_size),
        )

        depth_path = os.path.join(co3d_dir, f_data["depth"]["path"])
        assert f_data["depth"]["scale_adjustment"] == 1.0
        ...
        cx, cy = camera_intrinsics_np[:2, 2].round().astype(int)
        ...
        input_rgb_image, depth_mask, input_camera_intrinsics = crop_image_depthmap(
            input_rgb_image, depth_mask, camera_intrinsics_np, crop_bbox
        )
        ...
        camera_pose[:3, :3] = R.numpy()
        camera_pose[:3, 3] = tvec.numpy()
        camera_pose = np.linalg.inv(camera_pose)

        save_img_path = os.path.join(output_dir, filepath)
        save_depth_path = os.path.join(output_dir, f_data["depth"]["path"])
    ...
    for k in list(selected_sequences_numbers_dict.keys()):
        frames = sorted(set(selected_sequences_numbers_dict[k]))
        selected_sequences_numbers_dict[k] = frames
...
def main():
    ...
    for split in splits:
        ...
        if os.path.isfile(selected_sequences_path):
            with open(selected_sequences_path, "r") as file:
                all_selected_sequences = json.load(file)
        else:
        ...
        for category, seq_dict in all_selected_sequences.items():
            if category not in valid_sequences:
                valid_sequences[category] = {}
            for seq_name, frames in seq_dict.items():
                if seq_name not in valid_sequences[category]:
                    valid_sequences[category][seq_name] = []
                valid_sequences[category][seq_name].extend(frames)

    for category, seq_dict in valid_sequences.items():
        category_output_dir = osp.join(args.output_dir, category)
        os.makedirs(category_output_dir, exist_ok=True)
        cleaned_seq_dict = {}
        for seq_name, frames in seq_dict.items():
            frames_clean = sorted(set(int(f) for f in frames))
            if len(frames_clean) == 0:
                continue
            cleaned_seq_dict[seq_name] = frames_clean
        valid_seq_path = os.path.join(category_output_dir, "valid_seq.json")
        with open(valid_seq_path, "w") as file:
            json.dump(cleaned_seq_dict, file)
