...
def split_image_paths_evenly(image_paths: List[Path]) -> Tuple[List[Path], List[Path]]:
    ...
    已废弃，请使用 select_train_test_images。
    """
    set_a = image_paths[::2]
...
def select_train_test_images(
    ...
    Args:
        ...
        num_train: 训练集图像数量。如果为 None，则使用一半图像
        train_distribution: 训练集采样策略
            - "ordered": 按顺序选择前 num_train 个图像
            - "uniform": 均匀随机选择 num_train 个图像
        seed: 随机种子（当 train_distribution="uniform" 时使用）
    
    Returns:
        (train_paths, test_paths): 训练集和测试集的图像路径
    """
    total = len(image_paths)
    if total < 2:
        raise ValueError(f"至少需要 2 张图像，当前只有 {total} 张")
    
    # 确定训练集大小
    if num_train is None:
        num_train = total // 2
    num_train = min(num_train, total - 1)  # 至少保留 1 张作为测试集
    
    if train_distribution == "ordered":
        # 按顺序选择前 num_train 个
        train_paths = image_paths[:num_train]
        test_paths = image_paths[num_train:]
    elif train_distribution == "uniform":
        # 均匀随机选择
        import random
        rng = random.Random(seed)
        indices = list(range(total))
        train_indices = sorted(rng.sample(indices, num_train))
        train_set = set(train_indices)
        test_indices = [i for i in range(total) if i not in train_set]
        
        train_paths = [image_paths[i] for i in train_indices]
        test_paths = [image_paths[i] for i in test_indices]
    else:
        raise ValueError(
            f"不支持的 train_distribution: {train_distribution}. "
            f"支持的选项: 'ordered', 'uniform'"
        )
...
def run_ttt(model: AnySplat, cfg: TTTConfig) -> None:
    ...
    print(
        ...
        f"训练集: {len(train_images)} ({cfg.train_distribution}) | "
