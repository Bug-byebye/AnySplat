...
def setup_args():
    ...
    parser.add_argument('--categories', type=str, default='auto', help='Comma-separated categories, or "auto" to detect from *_test.jgz')
...
def process_sequence(
    ...
    Args:
        ...
        frame_ids: Optional fixed frame indices; when set, rng is not used for sampling.
        pose_align: "gt_only" (default), "both", or "none" — how to align cameras before relative pose error.
    ...
    if frame_ids is not None:
        ...
        if ids.shape != (num_frames,):
            raise ValueError(f"frame_ids must have shape ({num_frames},), got {ids.shape}")
    else:
    ...
    with torch.cuda.amp.autocast(dtype=torch.float32):
        ...
        if pose_align == "gt_only":
            gt_se3 = align_to_first_camera(gt_se3)
        elif pose_align == "both":
            pred_se3 = align_to_first_camera(pred_se3)
            gt_se3 = align_to_first_camera(gt_se3)
        elif pose_align == "none":
            pass
        else:
            raise ValueError(f"Unknown pose_align: {pose_align}")
...
def evaluate(args: argparse.Namespace):
    ...
    if args.categories.strip().lower() == "auto":
        ...
        categories = sorted(
            p.name[:-9] for p in anno_dir.glob("*_test.jgz") if p.name.endswith("_test.jgz")
        )
    else:
    ...
    for category in categories:
        ...
        for seq_name, seq_data in annotation.items():
            ...
            seq_rError, seq_tError = process_sequence(
                ...
                args.min_num_images, args.num_frames, args.use_ba, device, torch.bfloat16, rng
            )
